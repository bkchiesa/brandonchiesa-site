...
| Token | Value | Notes |
|-------|-------|-------|
| `font-family` | `"DejaVu Serif", "Liberation Serif", "PT Serif", Georgia, "Times New Roman", serif` | Comp used **DejaVu Serif Regular**. Production uses **PT Serif**. |
...
## Hover underline (Pixel PASS brass)

**Thickness:** ~**6px** visual weight @1280 CSS px (3× the prior ~2px cream bar). Scale with viewport (`0.47vw` ≈ 6/1280). Soft alpha edges; optical core ≈6px inside a ~10px strip.
...
| File | Size | Role |
|------|------|------|
| `public/art/v3/menu/mist_underline_brass.png` | 192×10 RGBA | Soft-edged horizontal strip; scale to label width |
| `public/art/v3/menu/mist_underline_brass_tile.png` | 64×10 RGBA | Optional 1D-seamless tile for `background-repeat: repeat-x` |

- Labels stay mist cream (`#F3EEE4`); only the underline is brass.
- Soft edges live in the PNG alpha — prefer texture over a hard CSS `border-bottom`.
- Hover **and selected** share this bar. Do **not** ship a cream hairline.
...
| `public/art/v3/menu/mist_hover_make_on_loop.png` | Same + **~6px faded brass** underline under **Make** only |
| `public/art/v3/menu/mist_contact.png` | Stacked idle \| hover contact with captions |
| `public/art/v3/menu/mist_underline_brass.png` | RGBA brass grain strip for CSS `::after` |
| `public/art/v3/menu/mist_underline_brass_tile.png` | Optional seamless 1D tile |
...
## Soft concerns
- DejaVu is the **comp** face; production uses PT Serif.
